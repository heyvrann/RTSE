...
finite_float = st.floats(min_value=-1e6, max_value=1e6,
                         allow_nan=False, allow_infinity=False, width=64)

@st.composite
def box2s(draw):
    x1 = draw(finite_float); y1 = draw(finite_float)
    x2 = draw(finite_float); y2 = draw(finite_float)
...
def pairs_unique_ids(draw, min_size=0, max_size=60):
    ...
    return draw(st.lists(st.tuples(box2s(), ids),
...
def nested_query_boxes(draw):
    ...
    y2 = draw(st.floats(min_value=ymin, max_value=ymax, allow_nan=False, allow_infinity=False))
    return (rtse.Box2(rtse.Point2(x1, y1), rtse.Point2(x2, y2)),
...
@given(st.lists(box2s(), min_size=0, max_size=25), 
       nested_query_boxes())
@settings(deadline=None)
def test_prop_monotonically_nested_queries(boxes, query_pairs):
    ...
    for id, box in enumerate(boxes):
        tree.insert(box, id)
    assert set(tree.query_range(query_small)).issubset(set(tree.query_range(query_large)))